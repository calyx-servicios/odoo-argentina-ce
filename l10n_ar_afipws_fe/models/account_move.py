...
class AccountMove(models.Model):
    ...
    def do_pyafipws_request_cae(self):
        ...
        for inv in self:
            ...
            if afip_ws == "wsfex":
                if not country:
                    raise UserError(_('For WS "%s" country is required on partner' % (afip_ws)))
            ...
            ws_next_invoice_number = (
                int(
                    inv.journal_id.get_pyafipws_last_invoice(inv.l10n_latam_document_type_id)[
                        "result"
                    ]
                )
                + 1
            ...
            #     )
                ...
                fecha_pago = (
                    ...
                    if int(doc_afip_code) == 19 and tipo_expo in [2, 4] and inv.invoice_date_due
                    else ""
                )
                ...
                if nro_doc:
                    ...
                    if not cuit_pais_cliente:
                        raise UserError(
                            _("No vat defined for the partner and also no CUIT " "set on country")
                        )
            ...
            if afip_ws != "wsfe":
                for line in inv.invoice_line_ids.filtered(lambda x: not x.display_type):
                    ...
                    bonif = line.discount and str("%.2f" % (precio * qty - importe)) or None
                    if afip_ws in ["wsmtxca", "wsbfe"]:
                        ...
                        if afip_ws == "wsmtxca":
                            ...
                            ws.AgregarItem(
                                codigo, sec, ds, qty, umed, precio, bonif, iva_id, importe + imp_iva
                            )
                    elif afip_ws == "wsfex":
                        ws.AgregarItem(codigo, ds, qty, umed, precio, "%.2f" % importe, bonif)
